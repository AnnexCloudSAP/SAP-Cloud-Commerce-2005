...
<div class="js-cart-totals row">
	...
	<c:if test="${cartData.totalDiscounts.value > 0}">
     ...
     <c:if test="${cartData.annexLoyaltyCredit.value > 0}">
         		<div class="col-xs-6 cart-totals-left discount">
         			<spring:theme code="basket.page.annex.totals.discounts"/>
         		</div>
         		<div class="col-xs-6 cart-totals-right text-right discount">
         			<ycommerce:testId code="Order_Totals_Savings">
         				<format:price priceData="${cartData.annexLoyaltyCredit}" displayNegationForDiscount="true" />
         			</ycommerce:testId>
         		</div>
      </c:if>
    ...
    <c:if test="${not empty earnAnnexLoyaltyPoints && earnAnnexLoyaltyPoints> 0}">
    	<div class="cart-totals-taxes text-right">
		<ycommerce:testId code="Annex_Cloud_Total">	<spring:theme code="basket.page.annex.earn.points.carts"/>
		   <p class="annex_promotion_pdp"><b>
				 ${earnAnnexLoyaltyPoints} &nbsp;<spring:theme code="annex.point"/></b></p>
		</ycommerce:testId>
		</div>
	</c:if>
