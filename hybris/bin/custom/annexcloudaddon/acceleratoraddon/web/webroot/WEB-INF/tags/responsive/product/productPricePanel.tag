...
<fmt:parseNumber var="acCost" integerOnly = "true" type = "number" value = "${productCosts}" />
<fmt:parseNumber var="acPoints" integerOnly = "true" type = "number" value = "${productPoints}" />
	...
	<c:when test="${empty product.volumePrices}">
		<c:choose>
			...
			<c:otherwise>
				...
				<c:if test = "${productCosts > 0.0}">
				<p><b>or Pay 
				  ${acCost} Points</b></p>
				</c:if>				
				<c:if test = "${productPoints > 0.0}">
				<p class="annex_promotion_pdp"><b>
				<c:if test="${not empty tierMultiplier}" >
				<spring:theme code="annex.tier.promotion.applied" /> &nbsp;<b>x${tierMultiplier}</b><br>
				</c:if>
				<spring:theme code="basket.page.annex.earn.points.carts" /> &nbsp;${acPoints} &nbsp;<spring:theme code="annex.point" /></b></p>
				</c:if>
			</c:otherwise>
		</c:choose>
	</c:when>
